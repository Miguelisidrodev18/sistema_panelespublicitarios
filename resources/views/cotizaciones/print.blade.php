...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .header-contact i-row { display: flex; align-items: center; gap: 4px; }
        ...
        .client-value strong { font-weight: 700; }
        ...
        .totals-total .t-label { font-size: 11px; font-weight: 700; letter-spacing: 0.5px; }
        .totals-total .t-value { font-size: 14px; font-weight: 900; color: #FCD34D; }

        /* ── Sección observaciones + pagos ── */
        .bottom-section {
            display: flex;
            gap: 14px;
            margin-bottom: 16px;
        ...
        .print-btn {
            ...
            font-weight: 700;
            cursor: pointer;
            box-shadow: 0 4px 12px rgba(220,30,46,.4);
        }
        .print-btn:hover { background: #B01825; }
        .close-btn {
            background: #374151;
            color: #fff;
            border: none;
...
<div class="print-btn-bar">
    <a href="{{ route('cotizaciones.show', $cotizacion) }}" class="close-btn">&#8592; Volver</a>
...
<div class="page">
        ...
        <div class="header-left">
            ...
            <div class="header-logo">
                @if(file_exists(public_path('images/logo.png')))
                    <img src="{{ asset('images/logo.png') }}" alt="{{ $empresa_propia['nombre'] }}">
                @else
        ...
        <div class="header-right">
            ...
            <table class="cot-info-table">
                <tr>
                    ...
                    <td class="val">{{ $cotizacion->numero }}</td>
                </tr>
                <tr>
                    <td class="lbl">FECHA</td>
                    <td class="val-dark">{{ $cotizacion->fecha_cotizacion?->format('d/m/Y') ?? now()->format('d/m/Y') }}</td>
                </tr>
                <tr>
                    ...
                    <td class="val-dark">
                        ...
                        @if($cotizacion->fecha_vencimiento)
                            ...
                            <span style="color:#64748B;font-size:9px">({{ max(0,$dias) }} días)</span>
                        @endif
                    </td>
                </tr>
                <tr>
                    <td class="lbl">RUC</td>
                    <td class="val-dark">{{ $empresa_propia['ruc'] }}</td>
                </tr>
            </table>
        </div>
    </div>
            ...
            <div class="client-box">
                <div class="client-data">
                    ...
                    <div class="client-row">
                        ...
                        <div class="client-value"><strong>{{ $nombre }}</strong></div>
                    </div>
                    @if($ruc)
                    <div class="client-row">
                        <div class="client-label">RUC:</div>
                        <div class="client-value">{{ $ruc }}</div>
                    </div>
                    @endif
                    @if($contacto)
                    <div class="client-row">
                        <div class="client-label">Contacto:</div>
                        <div class="client-value">{{ $contacto }}</div>
                    </div>
                    @endif
                    @if($telefono)
                    <div class="client-row">
                        <div class="client-label">Teléfono:</div>
                        <div class="client-value">{{ $telefono }}</div>
                    </div>
                    @endif
                    @if($email)
                    <div class="client-row">
                    ...
                    @endif
                    @if($cotizacion->tipo_contrato)
                    <div class="client-row">
                        <div class="client-label">Servicio:</div>
                        <div class="client-value"><span class="badge badge-led">{{ $cotizacion->tipo_contrato }}</span></div>
                    </div>
                    @endif
                </div>
        ...
        <div class="col-conditions">
            ...
            <div class="conditions-box">
                <div class="condition-item">
                    ...
                    <span>Sujeto a disponibilidad del espacio publicitario.</span>
                </div>
                <div class="condition-item">
                    <div class="condition-dot">&#10003;</div>
                    <span>La reserva se formaliza con el 50% de adelanto.</span>
                </div>
                <div class="condition-item">
                    <div class="condition-dot">&#10003;</div>
                    <span>El arte final debe enviarse con 3 días de anticipación.</span>
                </div>
                <div class="condition-item">
                    <div class="condition-dot">&#10003;</div>
                    <span>Los precios incluyen instalación y mantenimiento básico.</span>
                </div>
                <div class="condition-item">
                    <div class="condition-dot">&#10003;</div>
                    <span>Cotización válida por {{ $cotizacion->fecha_vencimiento ? now()->diffInDays($cotizacion->fecha_vencimiento, false) . ' días' : '15 días' }} desde la fecha de emisión.</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <table class="detail-table">
        <thead>
            <tr>
                ...
                <th style="width:62px">Foto</th>
                <th style="width:62px">Medidas</th>
                <th style="width:42px">Tipo</th>
                <th style="width:68px">Período</th>
                <th style="width:28px">Cant.</th>
                <th style="width:72px">Precio Unit.</th>
        ...
        <tbody>
            ...
            @php
                ...
                $nombre_item = $panel ? ($panel->nombre ?? $elem->codigo) : ($servicio ? $servicio->nombre : $elem->codigo);
                ...
                $tipo_badge = match($elem->tipo_elemento) {
                    'digital'    => ['badge-led',     'LED'],
                    'tradicional'=> ['badge-banner',  'BANNER'],
                    'servicio'   => ['badge-servicio','SERVICIO'],
                    default      => ['badge-led',     strtoupper($elem->tipo_elemento)],
                };
            ...
            <tr>
                ...
                <td class="left">
                    ...
                    @if($nombre_item && $nombre_item !== $elem->codigo)
                        <br><span style="color:#64748B;font-size:9.5px">{{ $nombre_item }}</span>
                    @endif
                </td>
                ...
                <td><span class="badge {{ $tipo_badge[0] }}">{{ $tipo_badge[1] }}</span></td>
                <td style="font-size:9.5px">{{ $periodo }}</td>
                <td>1</td>
                <td class="text-right fw-700">S/ {{ number_format($precio, 2, '.', ',') }}</td>
                <td class="text-right fw-700 text-green">S/ {{ number_format($precio, 2, '.', ',') }}</td>
            </tr>
            @if($elem->observaciones)
            <tr>
                <td></td>
                <td colspan="9" class="left" style="color:#64748B;font-size:9.5px;padding-top:2px;padding-bottom:6px;font-style:italic">
            ...
            @empty
            <tr>
                <td colspan="10" style="text-align:center;padding:20px;color:#94A3B8">Sin elementos detallados</td>
            </tr>
            @endforelse
        </tbody>
    </table>
    ...
    @php
        ...
        $total = $subtotal + $igv;
        // Si monto_propuesto difiere significativamente, usarlo como total final
        $monto_manual = (float) $cotizacion->monto_propuesto;
        if ($monto_manual > 0 && abs($monto_manual - $total) > 1) {
            $total     = $monto_manual;
            $subtotal  = $monto_manual / (1 + $igv_pct);
            $igv       = $monto_manual - $subtotal;
        }
    @endphp
    ...
    <div class="signatures">
        <div class="sign-block">
            ...
            <div class="sign-label">FIRMA DEL CLIENTE</div>
            <div class="sign-sub">{{ $nombre ?? 'Cliente' }}</div>
            @if($ruc ?? false)<div class="sign-sub">RUC: {{ $ruc }}</div>@endif
        </div>
        ...
        <div class="sign-block">
            <div class="sign-line"></div>
            <div class="sign-label">{{ $empresa_propia['razon_social'] }}</div>
            <div class="sign-sub">RUC: {{ $empresa_propia['ruc'] }}</div>
        </div>
    </div>
